...
def make_static_synthetic_world(
    ...
    *,
    ...
    source_shifts = {source: rng.normal(0.0, 0.18, size=5) for source in source_names}
    ...
    for source_index, source in enumerate(source_names):
        for participant_index in range(participants_per_dataset):
            ...
            person = _participant_truth(world_id, rng)
            for session_number in range(1, sessions_per_participant + 1):
                ...
                for window_number in range(1, n_windows + 1):
                    ...
                    latent = _window_latent(world_id, person, session_number, window_number, rng)
                    feature_vector = _feature_vector(world_id, latent, source_shifts[source], task_id, rng)
                    rows.append(
                        _canonical_row(
                            world_id=world_id,
                            checksum=checksum,
...
def _participant_truth(world_id: StaticWorldId, rng: np.random.Generator) -> dict[str, object]:
    trait = float(rng.normal(0.0, 0.65))
    tilt = float(rng.normal(0.0, 0.55))
    readiness = float(rng.normal(0.0, 0.9))
    component = None
...
def _feature_vector(
    ...
    noise = rng.normal(0.0, [0.025, 18.0, 0.00008, 0.018, 0.003])
